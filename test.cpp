...
bool Snake::move(bool circle)
{
    ...
    if (_move == SnakeMove::left)   body[0].x--; 
    if (_move == SnakeMove::right)  body[0].x++; 
    if (_move == SnakeMove::up)     body[0].y--; 
    if (_move == SnakeMove::down)   body[0].y++; 

    if (circle)
    {
        if (body[0].x < 0) body[0].x = max_x-1;
        if (body[0].x == max_x) body[0].x = 0;
        if (body[0].y < 0) body[0].x = max_y-1;
    ...
    {
        if ((body[i].x == body[0].x) && (body[i].y = body[0].y)) return false;
    }
    //*/
    return true;
}
